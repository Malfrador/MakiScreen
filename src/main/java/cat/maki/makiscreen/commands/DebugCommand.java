...
import cat.maki.makiscreen.video.FrameProcessor;
...
public class DebugCommand extends ECommand {
    ...
    public DebugCommand() {
        ...
        setPermission("makiscreen.debug");
        setPlayerCommand(true);
        setConsoleCommand(false);
        setMinArgs(0);
        setMaxArgs(4);
        setHelp("/maki debug <screen> [setting] [value]");
    }
    ...
    public void onExecute(String[] args, CommandSender sender) {
        ...
        if (args.length < 2) {
            sender.sendMessage(MM.deserialize("<red>Usage: /maki debug <screen> [setting] [value]"));
        ...
        switch (setting) {
            ...
            case "error-threshold", "threshold", "et" -> {
                if (args.length < 4) {
                    ...
                    sender.sendMessage(MM.deserialize("<gray>Usage: /maki debug " + screenName + " error-threshold <0-255>"));
                } else {
                    try {
            ...
            case "temporal-threshold", "tt" -> {
                if (args.length < 4) {
                    ...
                    sender.sendMessage(MM.deserialize("<gray>Usage: /maki debug " + screenName + " temporal-threshold <0-255>"));
                } else {
                    try {
                        int value = Integer.parseInt(args[3]);
            ...
            case "error-diffusion", "diffusion", "ed" -> {
                if (args.length < 4) {
                    ...
                    sender.sendMessage(MM.deserialize("<gray>Usage: /maki debug " + screenName + " error-diffusion <0.0-1.0>"));
                } else {
                    try {
    ...
    private void toggleDebugDisplay(Player player, VideoPlayer videoPlayer, String screenName) {
        ...
        if (currentlyEnabled) {
            ...
            player.sendMessage(MM.deserialize("<gray>Use <white>/maki debug " + screenName + " show</white> to see optimization settings"));
        }
    }
    ...
    private void showOptimizationStatus(CommandSender sender, FrameProcessor processor, String screenName) {
        ...
        sender.sendMessage(MM.deserialize(""));
        sender.sendMessage(MM.deserialize("<aqua>Tip: Toggle with /maki debug " + screenName + " <setting>"));
    }
